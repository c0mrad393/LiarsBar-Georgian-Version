...
let rounds = 0, steps = 0, calls = 0, maxRounds = 0;
const wins = [0, 0, 0, 0];
...
function check(s) {
  ...
  if (cards > 24) throw new Error("card duplication");
...
for (let g = 0; g < GAMES; g++) {
  ...
  let now = 0;
  let s = createGame(seats, { turnMs: 30000, pullMs: 15000 }, now);
...
console.log(`${GAMES} games OK · avg ${(rounds / GAMES).toFixed(1)} rounds (max ${maxRounds}) · ${steps} steps · ${calls} calls · wins by seat ${wins.join("/")}`);
